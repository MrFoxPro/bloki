...
import { RadioLine } from '@/components/radioline/radioline'
import { getBuildInfo } from '@/lib/build_info'
import { DropdownInput } from '@/components/dropdown/dropdown'
import { Switch } from '@/components/switch/switch'

const Appearance = () => {
   const { theme, transitTheme } = useThemeContext()
   const LANG_OPTIONS = languages.map((lang) => ({
      key: lang,
      label: LANGS_META[lang].name_eng,
      prefix: LANGS_META[lang].emoji,
   }))

   return (
      <>
         <fieldset class='h-57px w-100% lt-sm:flex-col flex p-x-80px items-center justify-between p-4 pt-2 '>
            <DropdownInput
