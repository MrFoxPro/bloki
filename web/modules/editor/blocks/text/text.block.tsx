...
import { TextBlock as TextBlockEntity } from '../../misc'
...
type TextBlockProps = {} & CommonContentProps<TextBlockEntity>

export function TextBlock(props: TextBlockProps) {
   let contentRef: HTMLDivElement
   let minTextWidth = 0
   let textHeightAtMinWidth = 0
   const { editor, setEditorStore, gridSize } = useEditorContext()
   ...
   createEffect(
      on(
         ...
         () => {
            ...
            const size = getTextBlockSize(
               props.block.type,
               props.block.fontFamily,
               props.block.value,
               editor.document.options.grid,
               props.block.width,
               'break-word'
            )
   ...
   function onTextInput(e: Event, pasteContent: string = null) {
      ...
      if (text === '' && props.block.width >= mGridWidth) {
         const boundSize = getTextBlockSize(
            props.block.type,
            props.block.fontFamily,
            text,
            editor.document.layoutOptions
         )
