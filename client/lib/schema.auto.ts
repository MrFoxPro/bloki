...
export type MeQuery = {
   ...
   me?: {
      ...
      workspaces: Array<{
         ...
         id: number;
         title: string;
         documents: Array<{ __typename?: 'BlokiDocument'; id: number; workspaceId: number; title: string; shared: boolean }>;
      }>;
   } | null;
};
...
export function getSdk(client: GraphQLClient, withWrapper: SdkFunctionWrapper = defaultWrapper) {
   return {
      me(variables?: MeQueryVariables, requestHeaders?: Dom.RequestInit['headers']): Promise<MeQuery> {
         return withWrapper(
            (wrappedRequestHeaders) => client.request<MeQuery>(MeDocument, variables, { ...requestHeaders, ...wrappedRequestHeaders }),
            'me',
            'query'
         );
      }
